...
class DepositController extends Controller
{
    ...
    public function all(){
        ...
        $deposits = Deposit::where('status','!=',-1)->with($this->depoRelations)->paginate(getPaginate());
        return view('admin.deposit.log', compact('pageTitle', 'emptyMessage', 'deposits'));
    }
    ...
    public function approve(Request $request)
    {
        ...
        $bookedTicket->status_payment = 1;
        $bookedTicket->save();

        $bookedTicket2 = BookedTicket::where('id', $bookedTicket->book_id)->first();
        if($bookedTicket2){
            $bookedTicket2->status_payment = 1;
            $bookedTicket2->save();
        };
